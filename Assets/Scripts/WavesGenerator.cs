...
public class WavesGenerator : MonoBehaviour
{
    ...
    // in case we want to add more cascades we will have to use the array cascades
    public int[] cascades = new int[] {};
    ...
    // create array of integers with fixed values
    int[] lengthScale = new int[] { 250, 17, 5 };
    ...
    void InitialiseCascades()
    {
        ...
        for(int i = 0; i < lengthScale.Length; i++) 
        {
            Shader.SetGlobalFloat("LengthScale" + i.ToString(), lengthScale[i]);
        }
    ...
    void RequestReadbacks() {
        ...
        AsyncGPUReadback.Request(cascade1.Displacement, 0, TextureFormat.RGBAFloat, OnCompleteReadback);
        AsyncGPUReadback.Request(cascade2.Displacement, 0, TextureFormat.RGBAFloat, OnCompleteReadback);
    }
